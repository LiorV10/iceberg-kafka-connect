...
public class WorkerTest {
  ...
  /**
   * Verifies that source offsets are advanced for the flag record but NOT for records
   * that come after it in the same batch.  This ensures that when the partition is paused
   * at committable() time, the committed consumer position is exactly flag_offset + 1, so
   * Kafka Connect re-delivers the post-flag records from the correct position upon resume.
   */
  @Test
  public void testPostFlagRecordOffsetsNotAdvanced() {
    IcebergSinkConfig config = mock(IcebergSinkConfig.class);
    when(config.dynamicTablesEnabled()).thenReturn(true);
    when(config.tablesRouteField()).thenReturn(FIELD_NAME);
    when(config.flagKeyPrefix()).thenReturn(FLAG_PREFIX);
    ...
    TopicPartition tp = new TopicPartition(SRC_TOPIC_NAME, 0);
    Map<String, Object> flagValue = ImmutableMap.of(FIELD_NAME, TABLE_NAME);
    // flag at offset 5, post-flag records at offsets 6 and 7
    SinkRecord flagRec =
        new SinkRecord(SRC_TOPIC_NAME, 0, null, FLAG_PREFIX + "end", null, flagValue, 5L);
    SinkRecord postRec1 =
        new SinkRecord(SRC_TOPIC_NAME, 0, null, "key1", null,
            ImmutableMap.of(FIELD_NAME, "db.tbl"), 6L);
    SinkRecord postRec2 =
        new SinkRecord(SRC_TOPIC_NAME, 0, null, "key2", null,
            ImmutableMap.of(FIELD_NAME, "db.tbl"), 7L);

    worker.write(ImmutableList.of(flagRec, postRec1, postRec2));

    Committable committable = worker.committable();
    // Committed offset must be flag_offset + 1 = 6, NOT 8 (post-flag records must NOT advance it)
    assertThat(committable.offsetsByTopicPartition().get(tp).offset()).isEqualTo(6L);
  }

  /**
   * Verifies that records arriving after the flag in the same batch are skipped entirely
   * (not written to any table) so that the partition can be paused and Kafka will re-deliver
   * them from exactly the flag-offset + 1 when resumed.
   */
  @Test
  public void testPostFlagRecordsInSameBatchAreSkipped() {
    ...
    // Post-flag record must NOT be written to any table — it is deferred until resume
   ...
   * flag state when called with the matching table identifier, so that new records delivered
   * after resume are written to their natural destination.
   */
  @Test
  public void testFlagStateIsClearedAndPartitionsResumedOnFlagProcessed() {
    IcebergSinkConfig config = mock(IcebergSinkConfig.class);
    when(config.dynamicTablesEnabled()).thenReturn(true);
    when(config.tablesRouteField()).thenReturn(FIELD_NAME);
    when(config.flagKeyPrefix()).thenReturn(FLAG_PREFIX);
    ...
    // Activate flag state by writing a flag record for TABLE_NAME
    Map<String, Object> flagValue = ImmutableMap.of(FIELD_NAME, TABLE_NAME);
    SinkRecord flagRec =
        new SinkRecord(SRC_TOPIC_NAME, 0, null, FLAG_PREFIX + "end", null, flagValue, 1L);
    worker.write(ImmutableList.of(flagRec));
   ...
   * identifier than the one the worker is currently waiting to process.  This ensures a per-table
   * sentinel for another table does not accidentally resume a worker that is waiting for its own table.
   */
  @Test
  public void testOnFlagProcessedIsNoOpForDifferentTable() {
    ...
    // Activate flag state by writing a flag for TABLE_NAME
    Map<String, Object> flagValue = ImmutableMap.of(FIELD_NAME, TABLE_NAME);
    SinkRecord flagRec =
        new SinkRecord(SRC_TOPIC_NAME, 0, null, FLAG_PREFIX + "end", null, flagValue, 1L);
    worker.write(ImmutableList.of(flagRec));

    // Signal flag processed for a DIFFERENT table — this worker's state must not be cleared
    ...
    // A record delivered while still in paused state must be skipped (not written)
    SinkRecord postRec =
        new SinkRecord(SRC_TOPIC_NAME, 0, null, "key", null,
   ...
   * destination, while post-flag records are skipped entirely (deferred until resume).
   */
  @Test
  public void testPreFlagRecordsWrittenNormallyPostFlagRecordsSkipped() {
    ...
    // Post-flag record must NOT be written to any table — deferred until partition is resumed
    verify(writerFactory, never()).createWriter(eq(postTable), any(), anyBoolean());
    // The flag branch table writer is also never created (flag itself is not routed via writerFactory)
    verify(writerFactory, never()).createWriter(eq(TABLE_NAME), any(), anyBoolean());
  }
