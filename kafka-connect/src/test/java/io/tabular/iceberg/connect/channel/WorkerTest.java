...
public class WorkerTest {
  ...
  /**
   * Verifies the DR-restart scenario: when a pod restarts and the flag is re-read (because the
   * flag's offset was intentionally not committed to preserve the paused state), {@code pause()}
   * must NOT be called until {@code committable()} is invoked.  This ensures that Kafka Connect
   * keeps calling {@code put()} after the restart so the coordinator's {@code START_COMMIT} can
   * be received and {@code committable()} can be called — committing the flag result.
   */
  @Test
  public void testPauseIsDeferredToCommittableAfterRestart() {
    IcebergSinkConfig config = mock(IcebergSinkConfig.class);
    when(config.dynamicTablesEnabled()).thenReturn(true);
    when(config.tablesRouteField()).thenReturn(FIELD_NAME);
    when(config.flagKeyPrefix()).thenReturn(FLAG_PREFIX);
    ...
    // Simulate a pod restart: create a fresh Worker (state reset)
    Worker freshWorker = new Worker(config, writerFactory, context);

    Map<String, Object> flagValue = ImmutableMap.of(FIELD_NAME, TABLE_NAME);
    SinkRecord flagRec =
        new SinkRecord(SRC_TOPIC_NAME, 0, null, FLAG_PREFIX + "end", null, flagValue, 5L);

    // Flag is re-read after restart — the partition must NOT be paused yet so Kafka Connect
    // continues to call put(), allowing the coordinator's START_COMMIT to be received and
    // committable() to be invoked.
    freshWorker.write(ImmutableList.of(flagRec));
    verify(context, never()).pause(tp);

    // committable() must include the flag result so the coordinator can process the flag
    Committable committable = freshWorker.committable();
    assertThat(committable.writerResults())
        .as("committable() must include the flag result after restart")
        .hasSize(1)
        .allMatch(r -> r instanceof FlagWriterResult);

    // Kafka-level pause is requested only now (deferred to committable() boundary)
    verify(context, times(1)).pause(tp);
  }
