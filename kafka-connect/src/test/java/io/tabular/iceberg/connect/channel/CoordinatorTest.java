...
public class CoordinatorTest extends ChannelTestBase {
  ...
  @Test
  public void testRecoveryCommitsOldResponsesWhenStartCommitIsReRead() {
    // Simulate crash recovery: old StartCommit + complete responses
    // are present on the control topic when the coordinator calls process()

    UUID oldCommitId = UUID.randomUUID();

    // Use high commitIntervalMs so process() won't start a new commit
    ...
    // Add old StartCommit, DataWritten, and DataComplete to control topic
    consumer.addRecord(
        new ConsumerRecord<>(
            CTL_TOPIC_NAME,
            0,
    ...
    // process() should resume the old commit and complete it
    coordinator.process();
    ...
    Map<String, String> summary = snapshot.summary();
    Assertions.assertEquals(oldCommitId.toString(), summary.get(COMMIT_ID_SNAPSHOT_PROP));
  }

  @Test
  public void testRecoveryPartialResponsesTimesOutToPartialCommit() {
    // Simulate crash recovery: old StartCommit + partial responses (not all partitions)
    // The coordinator should resume the old commit, then time out as a partial commit

    UUID oldCommitId = UUID.randomUUID();

    // Create members with 2 topic partitions so the coordinator expects 2 partition responses
    ...
    // Use high commitIntervalMs so process() won't start a new commit,
    // but 0 timeout so partial commit triggers immediately
    when(config.commitIntervalMs()).thenReturn(Integer.MAX_VALUE);
    ...
    // Add old StartCommit and responses from only 1 of 2 partitions
    consumer.addRecord(
        new ConsumerRecord<>(
            CTL_TOPIC_NAME,
            0,
    ...
    // Only 1 partition responds (out of 2 expected)
    consumer.addRecord(
        new ConsumerRecord<>(
            CTL_TOPIC_NAME,
            0,
    ...
    // process() should resume old commit, detect timeout, and do a partial commit
    coordinator.process();
