...
public class Coordinator extends Channel implements AutoCloseable {
  ...
  public void process() {
    if (commitState.isCommitIntervalReached()) {
      ...
      send(event);
      LOG.info(
          "Sent workers commit trigger with commit-id={}",
              ...
              if (targetBranch != null) {
                ...
                if (!pending.isEmpty()) {
                  ...
                  try {
                    ...
                    LOG.error(
                        "Failed to update types for table {}. {}",
                        table.name(),
                        e.getMessage());
                  }
                }
                ...
                try {
                  ...
                  LOG.info(
                      "Successfully switched branch for table {} to {}",
                      table.name(),
                      targetBranch);
                } catch (Exception e) {
                  LOG.error(
                      "Failed to switch branch for table {} to {}",
                      table.name(),
                      targetBranch,
                      e);
                }
